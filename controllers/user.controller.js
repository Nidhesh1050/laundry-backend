...
export async function registerUserController(request,response){
    try {
        const { name, email, phone, password, role, status } = request.body

        if(!name || !email || !password || !phone){
            return response.status(400).json({
                message : "provide email, name, password, phone",
                error : true,
                success : false
            })
        }
        ...
        if(user){
            return response.json({
                message : "This email already exist please check.",
                error : true,
                success : false
            })
        }
...
export async function logoutController(request,response){
    try {
        ...
        console.log('Logout called for user ID:', userid);
